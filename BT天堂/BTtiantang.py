...
def getlist(link):
    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        for url in urls:
            executor.submit(movie, url)

    nextpage = sel.xpath('//ul[@class="pagelist"]/li/a/text()')[-2]
    if nextpage == '下一页':
        return getlist(URL + sel.xpath('//ul[@class="pagelist"]/li/a/@href')[-2])
    ...
    if len(link) == len(infos):
        with ThreadPoolExecutor(max_workers=50) as executor:
            for i in range(len(link)):
                executor.submit(download, idpattern.search(link[i]).group(1), uhashpattern.search(link[i]).group(1),
                                infos[i])
